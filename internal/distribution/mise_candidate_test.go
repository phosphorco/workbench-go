...
func TestMiseGitHubBackendAutodetectsCandidateArchive(t *testing.T) {
	if goruntime.GOOS != "linux" && goruntime.GOOS != "darwin" {
		t.Skip("Workbench 0.2.0 does not distribute this operating system")
	}
	...
	assetName, err := AssetName("0.2.0", platform)
	if err != nil {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		assets := []map[string]any{
			{"name": "workbench@0.2.0.zip", "browser_download_url": baseURL + "/assets/contracts", "url": baseURL + "/api/assets/contracts"},
		}
		for _, name := range []string{
			"workbench-0.2.0-linux-arm64.tar.gz",
			"workbench-0.2.0-linux-x64.tar.gz",
			"workbench-0.2.0-macos-arm64.tar.gz",
			"workbench-0.2.0-macos-x64.tar.gz",
		} {
	...
	tool := fmt.Sprintf("github:phosphorco/workbench-go[api_url=%s,github_attestations=false]@0.2.0", server.URL)
	...
	installRoot := filepath.Join(data, "installs", "github-phosphorco-workbench-go", "0.2.0")
