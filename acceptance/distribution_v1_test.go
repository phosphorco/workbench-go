...
func TestDistributionV1ArchiveAndMiseCandidateNamesPublicBoundary(t *testing.T) {
	...
	assetName, err := distribution.AssetName("0.2.0", platform)
	if err != nil {
		t.Fatal(err)
	}
	...
	server.Config.Handler = http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		for _, name := range []string{
			"workbench-0.2.0-linux-arm64.tar.gz", "workbench-0.2.0-linux-x64.tar.gz",
			"workbench-0.2.0-macos-arm64.tar.gz", "workbench-0.2.0-macos-x64.tar.gz",
		} {
		...
		release := map[string]any{"tag_name": "0.2.0", "created_at": "2026-08-24T00:00:00Z", "draft": false, "prerelease": false, "assets": assets}
		switch request.URL.Path {
		case "/repos/phosphorco/workbench-go/releases":
			response.Header().Set("Content-Type", "application/json")
			_ = json.NewEncoder(response).Encode([]any{release})
		case "/repos/phosphorco/workbench-go/releases/tags/0.2.0":
			response.Header().Set("Content-Type", "application/json")
	...
	identity := fmt.Sprintf("github:phosphorco/workbench-go[api_url=%s,github_attestations=false]@0.2.0", server.URL)
	...
	installed := filepath.Join(data, "installs", "github-phosphorco-workbench-go", "0.2.0")
	...
	for _, marker := range []string{"mise install github:phosphorco/workbench-go@0.2.0", "workbench setup", "GH_TOKEN: \"\"", "GITHUB_TOKEN: \"\""} {
...
func distributionV1Inputs(t *testing.T, root string) distribution.ArchiveInputs {
	...
	return distribution.ArchiveInputs{
		Version: "0.2.0", Revision: snapshotPruneV1Commit,
