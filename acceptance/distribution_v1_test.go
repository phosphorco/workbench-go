...
func TestDistributionV1ArchiveAndMiseCandidateNamesPublicBoundary(t *testing.T) {
	...
	for _, marker := range []string{
		"mise install github:phosphorco/workbench-go@0.8.0",
		"workbench skills check",
		"workbench buildable resolve --name cold --platform linux-x86_64 --format json",
		`"digest":"[0-9a-f]{64}"`,
		"releases/download/0.8.0/workbench@0.8.0#/Repository.pkl",
		...
		"Consume a candidate sealed by published 0.6.1 with published 0.8.0",
		"published_revision:", "required: true", "fixture_revision:", "default: published-0.8.0", "EXPECTED_PUBLISHED_REVISION: ${{ inputs.published_revision }}", "FIXTURE_REVISION: ${{ inputs.fixture_revision }}", "ACCEPTANCE_REVISION=\"$FIXTURE_REVISION\"", "published binary provenance does not match expected published revision", "github:phosphorco/workbench-go@0.6.1",
		"releases/download/0.6.0/workbench@0.6.0#/Repository.pkl",
		"published 0.6.1 legacy manifest declarationIdentity=absent",
		"published 0.6.1 sealed output evidence=sha256", "published 0.8.0 consuming verify exit=",
		"published 0.8.0 consuming check-fresh exit=", "buildable materialize --name compat --platform linux-x86_64 --destination materialized",
		"bash acceptance/release_compatibility.sh",
		"command/scenario                  0.6.1 exit  0.8.0 exit", "python3 - \"$manifest\"",
	} {
	...
	if strings.Contains(workflow, "git clone --depth 1 --branch main https://github.com/phosphorco/monorepo.git") {
		t.Fatal("release acceptance must not clone the private monorepo; use the public 0.6.1 seal/0.8.0 consume witness")
	}
...
func TestReleaseWorkflowBuildsCompleteCandidateBeforeTagOnlyPublication(t *testing.T) {
	...
	for _, marker := range []string{
		...
		"candidate-checks:",
		"needs: [binaries, contracts, candidate-checks]",
		"Prove installed context activation, no-source behavior, and cleanup",
		`daemon_pids=$(lsof -t "$socket" | sort -u)`,
		"subject-path: candidate/out/workbench-*.tar.gz",
		"subject-path: contracts/workbench@0.8.0.zip",
		"gh release create",
	} {
		if !strings.Contains(workflow, marker) {
...
func TestPublishedCompatibilityMatrixOwnsIndependentOldAndNewExitChecks(t *testing.T) {
	...
	for _, marker := range []string{
		...
		"command/scenario                  0.6.1 exit  0.8.0 exit",
