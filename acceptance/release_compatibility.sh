...
printf 'command/scenario                  0.6.1 exit  0.8.0 exit  0.6.1 stdout  0.8.0 stdout\n'
for label in \
  ...
  new_status="$(< "$results_root/0.8.0-${label}.status")"
  old_bytes="$(< "$results_root/0.6.1-${label}.bytes")"
  new_bytes="$(< "$results_root/0.8.0-${label}.bytes")"
...
new_materialize_stdout="$results_root/0.8.0-materialize-success.stdout"
...
printf 'materialize receipt 0.6.1 stdout bytes=%s; 0.8.0 stdout:\n' "$(< "$results_root/0.6.1-materialize-success.bytes")"
